-- Fix JSONB field handling in svc_save_core_patch function
...
BEGIN
  ...
  -- Dynamic update using filtered patch with proper type handling for JSONB fields
  UPDATE services s
  SET 
    ...
    -- Use -> for JSONB fields, not ->>
    compliance_checklist = COALESCE((filtered_patch->'compliance_checklist'), s.compliance_checklist),
    regulatory_findings = COALESCE((filtered_patch->'regulatory_findings'), s.regulatory_findings),
    supporting_documents = COALESCE((filtered_patch->'supporting_documents'), s.supporting_documents),
    ...
    -- Use -> for JSONB fields, not ->>
    booking_time_rules = COALESCE((filtered_patch->'booking_time_rules'), s.booking_time_rules),
