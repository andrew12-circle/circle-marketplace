...
-- Create a view for public vendor directory (marketing purposes)
...
-- Create audit trigger for vendor data access
...
BEGIN
  ...
  IF TG_OP = 'SELECT' AND auth.uid() IS NOT NULL THEN
    INSERT INTO public.audit_log (
      ...
      'vendors_sensitive_access',
      'SELECT',
      auth.uid(),
      jsonb_build_object(
        'vendor_id', OLD.id,
        'accessed_columns', 'sensitive_data',
...
-- Note: Row-level triggers for SELECT operations have limitations in PostgreSQL
-- Instead, we'll rely on the RLS policies and functions for access control
...
COMMENT ON VIEW public.vendor_directory IS 'Public directory of approved vendors with non-sensitive information only';