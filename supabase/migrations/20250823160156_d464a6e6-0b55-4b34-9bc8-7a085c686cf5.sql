...
-- Note: Using regular CREATE INDEX since CONCURRENTLY can't run in transaction blocks
...
CREATE INDEX IF NOT EXISTS idx_service_views_service_created 
...
CREATE INDEX IF NOT EXISTS idx_content_engagement_events_content_created 
...
-- Optimize frequently used RPC functions by adding better query patterns
-- Update get_trending_services function to use better indexing strategy
...
BEGIN
  ...
  scored_services AS (
    SELECT 
      ...
      -- Simplified trending score: views + bookings + revenue impact
