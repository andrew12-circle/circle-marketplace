-- Update svc_save_core_patch function to include logo_url and profile_image_url fields
...
BEGIN
  ...
  IF p_patch ? 'logo_url' THEN set_clauses := array_append(set_clauses, 'logo_url = ' || quote_literal(p_patch->>'logo_url')); END IF;
  IF p_patch ? 'profile_image_url' THEN set_clauses := array_append(set_clauses, 'profile_image_url = ' || quote_literal(p_patch->>'profile_image_url')); END IF;
  ...
  IF p_patch ? 'time_to_results' THEN set_clauses := array_append(set_clauses, 'time_to_results = ' || quote_literal(p_patch->>'time_to_results')); END IF;
  ...
  IF p_patch ? 'pricing_tiers' THEN set_clauses := array_append(set_clauses, 'pricing_tiers = ' || quote_literal(p_patch->'pricing_tiers'::text) || '::jsonb'); END IF;
