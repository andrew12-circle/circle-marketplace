...
-- Working within public schema only (cannot modify auth schema)
...
-- Agent/vendor relationship indexes (only create if not exists)
...
-- Content and creator indexes (only if missing)
...
CREATE INDEX IF NOT EXISTS idx_marketplace_cache_expires ON public.marketplace_cache(expires_at) WHERE expires_at > now();
...
CREATE INDEX IF NOT EXISTS idx_login_attempts_email ON public.login_attempts(email, attempt_time DESC);
...
CREATE INDEX IF NOT EXISTS idx_review_responses_vendor_id ON public.review_responses(vendor_id);

CREATE INDEX IF NOT EXISTS idx_saved_services_user_id ON public.saved_services(user_id);

CREATE INDEX IF NOT EXISTS idx_security_events_user_id ON public.security_events(user_id);

CREATE INDEX IF NOT EXISTS idx_service_bookmarks_user_id ON public.service_bookmarks(user_id);

CREATE INDEX IF NOT EXISTS idx_service_inquiries_user_id ON public.service_inquiries(user_id);
CREATE INDEX IF NOT EXISTS idx_service_inquiries_vendor_id ON public.service_inquiries(vendor_id);
...
CREATE INDEX IF NOT EXISTS idx_vendor_reviews_reviewer_id ON public.vendor_reviews(reviewer_id);
