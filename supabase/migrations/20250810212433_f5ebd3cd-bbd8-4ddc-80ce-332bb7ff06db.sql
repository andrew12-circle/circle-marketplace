...
-- Get the first available service and some real user IDs
...
-- Update profiles for these users with review personas
WITH user_info AS (
  SELECT id as user_id, ROW_NUMBER() OVER () as rn FROM auth.users LIMIT 5
)
...
SELECT 
  u.user_id,
  CASE u.rn
    WHEN 1 THEN 'Jennifer Martinez'
    WHEN 2 THEN 'Robert Chen'
    WHEN 3 THEN 'Amanda Thompson'
    WHEN 4 THEN 'Michael Rodriguez'
    WHEN 5 THEN 'Sarah Williams'
  END as display_name,
  CASE u.rn
    WHEN 1 THEN 'Realty One Group'
    WHEN 2 THEN 'Century 21 Elite'
    WHEN 3 THEN 'Keller Williams Premier'
    WHEN 4 THEN 'RE/MAX Champions'
    WHEN 5 THEN 'Coldwell Banker Premier'
  END as business_name,
  CASE u.rn
    WHEN 1 THEN ARRAY['realtor', 'marketing']
    WHEN 2 THEN ARRAY['realtor', 'investment']
    WHEN 3 THEN ARRAY['realtor', 'residential']
    WHEN 4 THEN ARRAY['realtor', 'commercial']
    WHEN 5 THEN ARRAY['realtor', 'luxury']
  END as specialties,
  now() as created_at
FROM user_info u
