...
serve(async (req) => {
  ...
  try {
    const { vendor_id, pricing_url } = await req.json();

    if (!vendor_id || !pricing_url) {
      throw new Error('vendor_id and pricing_url are required');
    }

    console.log(`📸 Capturing pricing screenshot for vendor ${vendor_id} at ${pricing_url}`);
    ...
    const screenshotUrl = `vendor-pricing/${vendor_id}/${Date.now()}.png`;

    console.log(`💾 Uploading screenshot to storage: ${screenshotUrl}`);

    // Upload to Supabase Storage
    const { error: uploadError } = await supabase.storage
      .from('vendor-pricing-screenshots')
    ...
    // Update vendor record with pricing screenshot URL
    ...
    console.error('Error in capture-vendor-pricing function:', error);
    return new Response(JSON.stringify({
      error: error.message
    }), {
      status: 500,
