  ...
  return (
    <div className="flex items-center gap-1 sm:gap-2 lg:gap-3">
      ...
      {user && profile && (
        <Link 
          to="/wallet" 
          className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm hover:bg-accent hover:text-accent-foreground rounded-md px-1.5 sm:px-3 py-1.5 sm:py-2 transition-colors cursor-pointer touch-target min-w-0"
        >
      ...
      {user && profile && !profile.is_pro_member && (
        <Button 
          ...
          variant="secondary" 
          size="sm"
          className="hidden md:flex bg-gradient-to-r from-circle-primary to-circle-primary border-none text-white hover:from-circle-primary/90 hover:to-circle-primary/90 shadow-lg text-xs touch-target"
        >
