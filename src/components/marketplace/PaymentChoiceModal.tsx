...
export const PaymentChoiceModal = ({
  ...
  return <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            ...
            {(!isProMember || !service.pro_price) && <Card>
                <CardContent className="p-4">
                  ...
                  <Button onClick={handleRetailChoice} variant="outline" className="w-full" disabled={!user}>
                    {!user ? <>
            ...
            <Card className={!isProMember ? "opacity-75 border-dashed" : ""}>
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {isProMember ? <Badge variant="secondary" className="bg-primary/10 text-primary">
                      <Crown className="w-3 h-3 mr-1" />
                      Pro Member
                    </Badge> : <Badge variant="outline" className="border-orange-300 text-orange-600">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  {isProMember ? "Pay your discounted member price and get started immediately." : "Exclusive discounted pricing for Circle Pro members."}
                </p>
                <Button onClick={handleProChoice} className="w-full" disabled={!user || !isProMember}>
                  {!user ? <>
                      <UserPlus className="w-4 h-4 mr-2" />
                      Sign In to Add to Cart
                    </> : !isProMember ? <>
                ...
                {user && !isProMember && <div className="mt-2 text-center">
                    <p className="text-sm text-muted-foreground">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="font-semibold">Get Vendor Help (Match)</h3>
                    </div>
                    {isProMember ? <Badge variant="secondary" className="bg-primary/10 text-primary">
                        <Crown className="w-3 h-3 mr-1" />
                        Pro Member
            ...
            {agentPoints > 0 && !loadingPoints && <>
                ...
                <Card className={!isProMember ? "opacity-75 border-dashed" : ""}>
                  <CardContent className="p-4">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      {isProMember ? `${pointsBreakdown.respaNote} • ${agentPoints} available` : "Use your accumulated points to reduce service costs"}
                    </p>
                    <Button onClick={isProMember && pointsBreakdown.canUsePoints ? handlePointsChoice : undefined} disabled={!isProMember || !pointsBreakdown.canUsePoints} variant="outline" className="w-full">
                      {!isProMember ? <>
                          <Crown className="w-4 h-4 mr-2" />
                          Upgrade to Pro to Use Points
                        </> : "Use Points"}
                    </Button>
                    {!isProMember && <div className="mt-2 text-center">
                        <p className="text-sm text-muted-foreground">
                          <a href="/pricing" className="text-primary hover:underline font-medium">
                            Upgrade to Circle Pro
                          </a>{" "}
      ...
      {isVendorModalOpen && <VendorSelectionModal isOpen={isVendorModalOpen} onClose={() => setIsVendorModalOpen(false)} onVendorSelect={handleVendorSelected} service={service} />}
    </>;
};