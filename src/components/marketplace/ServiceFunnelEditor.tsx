...
import { Star, TrendingUp, Users, Target, Zap, Trophy, Building, Plus, X, Upload, Video, Image as ImageIcon, FileText, Monitor, Smartphone, DollarSign, CheckCircle, ArrowUp, ArrowDown, Grip, Shield, Award, Phone, Mail, Globe, Clock, Crown, ExternalLink, ShoppingCart } from 'lucide-react';
interface MediaItem {
  id: string;
...
export const ServiceFunnelEditor = ({
  funnelContent,
  onChange
}: ServiceFunnelEditorProps) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input id="headline" value={funnelContent.headline} onChange={e => updateContent('headline', e.target.value)} placeholder="Your compelling main headline" />
              </div>
              
              <div>
                <Label htmlFor="subheadline">Subheadline</Label>
                <Input id="subheadline" value={funnelContent.subheadline} onChange={e => updateContent('subheadline', e.target.value)} placeholder="Supporting subheadline" />
              </div>
              
              <div>
                <Label htmlFor="heroDescription">Description</Label>
                <Textarea id="heroDescription" value={funnelContent.heroDescription} onChange={e => updateContent('heroDescription', e.target.value)} placeholder="Detailed description of your service" rows={3} />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="roi">Estimated ROI (multiplier)</Label>
                  <Input id="roi" type="number" min="1" step="0.1" value={funnelContent.estimatedRoi} onChange={e => updateContent('estimatedRoi', parseFloat(e.target.value))} />
                </div>
                
                <div>
                  <Label htmlFor="duration">Duration/Timeline</Label>
                  <Input id="duration" value={funnelContent.duration} onChange={e => updateContent('duration', e.target.value)} placeholder="e.g., 30 days, 3 months" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input id="benefitsTitle" value={funnelContent.whyChooseUs.title} onChange={e => updateContent('whyChooseUs.title', e.target.value)} placeholder="Why choose your service?" />
              </div>
              ...
              <div className="space-y-4">
                ...
                {funnelContent.whyChooseUs.benefits.map((benefit, index) => <Card key={index}>
                    <CardContent className="p-4">
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          <Label>Icon</Label>
                          <Input value={benefit.icon} onChange={e => {
        ...
        <TabsContent value="media" className="space-y-6">
          <Card>
            ...
            <CardContent>
              <ServiceImageUpload value="" onChange={url => {
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                {funnelContent.media.map((item, index) => <Card key={item.id}>
                    <CardContent className="p-4">
        ...
        <TabsContent value="packages" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {funnelContent.packages.map((pkg, index) => <Card key={pkg.id}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <Label>Price ($)</Label>
                        <Input type="number" value={pkg.price} onChange={e => updatePackage(index, 'price', parseFloat(e.target.value))} />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <Input type="number" value={pkg.originalPrice || ''} onChange={e => updatePackage(index, 'originalPrice', e.target.value ? parseFloat(e.target.value) : undefined)} />
                    </div>

                    <div>
                      <Label>Savings Text (optional)</Label>
                      <Input value={pkg.savings || ''} onChange={e => updatePackage(index, 'savings', e.target.value)} placeholder="e.g., 50% OFF" />
                    </div>

                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        <Switch checked={pkg.popular} onCheckedChange={checked => updatePackage(index, 'popular', checked)} />
                        <Label>Most Popular</Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Switch checked={pkg.proOnly || false} onCheckedChange={checked => updatePackage(index, 'proOnly', checked)} />
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                {funnelContent.socialProof.testimonials.map((testimonial, index) => <Card key={testimonial.id}>
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <Input type="number" min="1" max="5" value={testimonial.rating} onChange={e => {
                            const newTestimonials = [...funnelContent.socialProof.testimonials];
              ...
              <div>
                ...
                {funnelContent.socialProof.stats.map((stat, index) => <Card key={index}>
                    <CardContent className="p-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label>Label</Label>
        ...
        <TabsContent value="cta" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Input value={funnelContent.trustIndicators.guarantee} onChange={e => updateContent('trustIndicators.guarantee', e.target.value)} placeholder="e.g., 30-Day Money Back Guarantee" />
                  </div>
                  <div>
                    <Label>Cancellation Policy</Label>
                    <Input value={funnelContent.trustIndicators.cancellation} onChange={e => updateContent('trustIndicators.cancellation', e.target.value)} placeholder="e.g., Cancel Anytime" />
                  </div>
                  <div>
                    <Label>Certification/Award</Label>
                    <Input value={funnelContent.trustIndicators.certification} onChange={e => updateContent('trustIndicators.certification', e.target.value)} placeholder="e.g., Industry Leader" />
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <Input value={funnelContent.callToAction.primaryHeadline} onChange={e => updateContent('callToAction.primaryHeadline', e.target.value)} placeholder="Need More Information?" />
                      </div>
                      <div>
                        <Label>Description</Label>
                        <Textarea value={funnelContent.callToAction.primaryDescription} onChange={e => updateContent('callToAction.primaryDescription', e.target.value)} placeholder="Visit our website for detailed documentation and resources." rows={2} />
                      </div>
                      <div>
                        <Label>Button Text</Label>
                        <Input value={funnelContent.callToAction.primaryButtonText} onChange={e => updateContent('callToAction.primaryButtonText', e.target.value)} placeholder="Visit Official Website" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <Label>Headline</Label>
                        <Input value={funnelContent.callToAction.secondaryHeadline} onChange={e => updateContent('callToAction.secondaryHeadline', e.target.value)} placeholder="Questions? We're Here to Help!" />
                      </div>
                      <div>
                        <Label>Description</Label>
                        <Textarea value={funnelContent.callToAction.secondaryDescription} onChange={e => updateContent('callToAction.secondaryDescription', e.target.value)} placeholder="Speak with our experts to find the perfect package for your business." rows={2} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Contact Information */}
              <div>
                <h4 className="font-semibold mb-4">Circle Pro Price ($)</h4>
                <div className="space-y-3">
                  <div>
                    <Label>Phone Number</Label>
                    <Input value={funnelContent.callToAction.contactInfo.phone} onChange={e => updateContent('callToAction.contactInfo.phone', e.target.value)} placeholder="(555) 123-4567" />
                  </div>
                  <div>
                    <Label>Email Address</Label>
                    <Input value={funnelContent.callToAction.contactInfo.email} onChange={e => updateContent('callToAction.contactInfo.email', e.target.value)} placeholder="support@yourcompany.com" />
                  </div>
                  <div>
                    <Label>Website URL</Label>
                    <Input value={funnelContent.callToAction.contactInfo.website} onChange={e => updateContent('callToAction.contactInfo.website', e.target.value)} placeholder="https://yourcompany.com" />
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center space-x-2 mb-4">
                  <Switch checked={funnelContent.urgency.enabled} onCheckedChange={checked => updateContent('urgency.enabled', checked)} />
                  <Label>Enable Urgency Message</Label>
                </div>
                
                {funnelContent.urgency.enabled && <div>
                    <Label>Urgency Message</Label>
                    <Input value={funnelContent.urgency.message} onChange={e => updateContent('urgency.message', e.target.value)} placeholder="Limited time offer - Act now!" />
                  </div>}
              </div>
            </CardContent>
          </Card>
              ...
              <div className="max-h-[600px] overflow-y-auto border rounded-lg bg-background">
                ...
                <div className="relative bg-gradient-to-br from-circle-primary to-circle-primary/80 text-white p-8">
                  <div className="flex items-start gap-6">
                    <div className="w-24 h-24 bg-white rounded-xl p-4 flex-shrink-0">
                      {funnelContent.media[0] ? <img src={funnelContent.media[0].url} alt={funnelContent.headline} className="w-full h-full object-contain" /> : <div className="w-full h-full bg-gray-200 rounded flex items-center justify-center text-xs text-gray-500">
                          No Image
                        </div>}
                    </div>
                ...
                <div className="p-8">
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      {funnelContent.packages.map(pkg => <Card key={pkg.id} className={`relative transition-all duration-200 ${pkg.popular ? 'ring-2 ring-circle-primary scale-105' : ''}`}>
                          ...
                          <CardContent className="p-6">
                            ...
                            <div className="text-center mb-4">
                              {pkg.originalPrice && <div className="text-sm text-muted-foreground line-through">
                                  ${pkg.originalPrice}
                                </div>}
                              <div className="text-3xl font-bold text-circle-primary">
                                ${pkg.price}
                              </div>
                              {pkg.savings && <Badge className="bg-destructive text-destructive-foreground text-xs mt-1">
                                  {pkg.savings} OFF
                                </Badge>}
                            </div>

                            <div className="space-y-2 mb-6">
                              {pkg.features.map((feature, featureIndex) => <div key={featureIndex} className="flex items-center gap-2 text-sm">
